...
//        }
        ...
        public boolean addMessage(ChatMessage pChatMessage)
        {
            boolean messageExist = messageExists(pChatMessage);

            if(!messageExist)
                messages.add(pChatMessage);

            return !messageExist;
        }

        public boolean messageExists(ChatMessage pChatMessage)
        {
            boolean exist = false;
            for (int i = 0; i < messages.size(); i++) {
                exist = messages.get(i).equals(pChatMessage);
                if(exist)
                    return exist;
            }
            return exist;
